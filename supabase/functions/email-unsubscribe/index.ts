...
function htmlPage(title: string, message: string, ok = true) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"/>
    ...
    <div class="card"><h1>${escapeHtml(title)}</h1><p>${message}</p><div class="brand">Palácios 3D Studio</div></div>
    </body></html>`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(htmlPage('Descadastro confirmado',
      `O endereço <b>${escapeHtml(tok.email)}</b> foi removido da nossa lista de envios. Você não receberá mais emails de campanhas.`),
