...
import { useAddClienteCEO, Parcela } from "@/hooks/useClientesCEO";
...
export default function ClienteFormModal({ open, onOpenChange }: Props) {
  const add = useAddClienteCEO();
  ...
  const canSave = empresa.trim() && projeto.trim() && valorNum > 0 && pctOk;

  const handleSave = async () => {
    const parcelasFinal: Parcela[] = parcelas.map((p, i) => ({
      ...p,
      data_prevista: previstas[i],
    }));
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-background/95 backdrop-blur-xl border-white/10">
        ...
        <div className="space-y-6">
          ...
          <section className="space-y-3">
            <h3 className="text-xs uppercase tracking-wider text-amber-400/90 font-semibold">Projeto</h3>
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2">
              {parcelas.map((p, i) => (
                <div
                  key={i}
                  className="rounded-xl p-3 bg-white/[0.03] border border-white/5 grid grid-cols-12 gap-2 items-center"
                >
            ...
            <div className="space-y-2">
              <ServicoRow checked={temImagens} onChecked={setTemImagens} label="Imagens">
                <Input
                  ...
                  placeholder="Qtd"
                  value={qtdImagens || ""}
                  onChange={(e) => setQtdImagens(Number(e.target.value) || 0)}
                />
              </ServicoRow>
              <ServicoRow checked={temAnimacao} onChecked={setTemAnimacao} label="Animação">
                <Input
                  ...
                  value={segAnimacao || ""}
                  onChange={(e) => setSegAnimacao(Number(e.target.value) || 0)}
                />
              </ServicoRow>
              <ServicoRow checked={temTour} onChecked={setTemTour} label="Tour virtual">
                <Input
                  ...
                  placeholder="Valor R$"
                  value={valorTour || ""}
                  onChange={(e) => setValorTour(Number(e.target.value) || 0)}
                />
              </ServicoRow>
              <ServicoRow checked={temAdic} onChecked={setTemAdic} label="Serviços adicionais">
                <div className="flex gap-2 flex-1">
                  <Input
                    ...
                    value={descAdic}
                    onChange={(e) => setDescAdic(e.target.value)}
                  />
                  <Input
                    type="number"
                    className="h-8 w-28 text-sm"
                    placeholder="R$"
                    value={valorAdic || ""}
                    onChange={(e) => setValorAdic(Number(e.target.value) || 0)}
                  />
                </div>
              </ServicoRow>
          ...
          <section className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              Palavras que aparecem nos pagamentos da planilha (separe por vírgula). O nome da empresa é incluído
              automaticamente.
            </p>
            <Input
              value={apelidosText}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={handleSave} disabled={!canSave || add.isPending} className="bg-amber-500/90 hover:bg-amber-500 text-black">
              {add.isPending ? "Salvando..." : "Salvar cliente"}
            </Button>
          </div>
        </div>
      </DialogContent>
