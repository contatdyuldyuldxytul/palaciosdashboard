        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="glass-card p-5">
          ...
          {checklist.length > 0 && (
            <div className="h-1.5 rounded-full bg-muted/30 overflow-hidden mb-4">
              <motion.div initial={{ width: 0 }} animate={{ width: `${checklist.length > 0 ? (completedCount / checklist.length) * 100 : 0}%` }} transition={{ duration: 0.8 }} className="h-full rounded-full bg-primary" />
            </div>
          )}
          {checklistLoading ? (
            ...
            <div className="space-y-1.5 max-h-[240px] overflow-y-auto scrollbar-thin">
              {checklist.map((item) => (
                <button key={item.id} onClick={() => handleToggleChecklist(item)}
                  ...
                  <div className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition-all ${item.concluida ? "bg-primary border-primary" : "border-muted-foreground/30"}`}>
